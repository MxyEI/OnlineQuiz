...
import com.shzu.m.CreateDOM;

@WebServlet(urlPatterns = { "/login", "/register", "/takeExam", "/logout","/insertQuestion" })
	...
	protected void doGet(HttpServletRequest request,
		...
		if (request.getRequestURI().equals(applicationContextPath + "/")) {
			...
			dispatcher.forward(request, response);
				...
				applicationContextPath + "/insertQuestion")) {
			RequestDispatcher dispatcher = request
					.getRequestDispatcher("/WEB-INF/jsps/insertQuestion.jsp");
			dispatcher.forward(request, response);
		}else if (request.getRequestURI().equals(
